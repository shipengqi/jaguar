...
func (s *GenericAPIServer) Setup() {
	gin.DebugPrintRouteFunc = func(httpMethod, absolutePath, handlerName string, nuHandlers int) {
		xlog.Infof("%-6s %-s ==> %s (%d handlers)", httpMethod, absolutePath, handlerName, nuHandlers)
	}
}
...
func (s *GenericAPIServer) InstallMiddlewares() {
	...
	for _, m := range s.middlewares {
		...
		if !ok {
			xlog.Warnf("can not find middleware: %s", m)
			continue
		}

		xlog.Infof("install middleware: %s", m)
		s.Use(mw)
	}
}
...
func (s *GenericAPIServer) Run() error {
	...
	eg.Go(func() error {
		xlog.Infof("Start to listening the incoming requests at http address: %s", s.InsecureServingInfo.Address)
		if err := s.insecureServer.ListenAndServe(); err != nil && !xerr.Is(err, http.ErrServerClosed) {
			return err
		}

		xlog.Infof("HTTP server at %s stopped", s.InsecureServingInfo.Address)
		return nil
	})
	...
	eg.Go(func() error {
		...
		xlog.Infof("Start to listening the incoming requests at https address: %s", s.SecureServingInfo.Address())
		if err := s.secureServer.ListenAndServeTLS(cert, key); err != nil && !xerr.Is(err, http.ErrServerClosed) {
			return err
		}

		xlog.Infof("HTTP server at %s stopped", s.SecureServingInfo.Address())
		return nil
	})
...
func (s *GenericAPIServer) Close(ctx context.Context) {
	if err := s.secureServer.Shutdown(ctx); err != nil {
		xlog.Warnf("Shutdown secure server failed: %s", err.Error())
	}

	if err := s.insecureServer.Shutdown(ctx); err != nil {
		xlog.Warnf("Shutdown insecure server failed: %s", err.Error())
	}
}
...
func (s *GenericAPIServer) ping(ctx context.Context) error {
	...
	for {
		...
		if err == nil && resp.StatusCode == http.StatusOK {
			xlog.Info("The router has been deployed successfully.")
			_ = resp.Body.Close()
			return nil
		}

		// Sleep for a second to continue the next ping.
		xlog.Info("Waiting for the router, retry in 1 second.")
		time.Sleep(1 * time.Second)

		select {
		case <-ctx.Done():
			xlog.Fatal("can not ping http server within the specified time interval.")
		default:
		}
	}
}
