...
func NewPool(opts *Options) *Client {
	...
	xlog.Debug("Creating new Redis connection pool")
	...
	if opts.MasterName != "" {
		xlog.Info("==> [REDIS] Creating sentinel-backed failover client")
		client = redis.NewFailoverClient(cfg.failover())
	} else if opts.EnableCluster {
		xlog.Info("==> [REDIS] Creating cluster client")
		client = redis.NewClusterClient(cfg.cluster())
	} else {
		xlog.Info("==> [REDIS] Creating single-node client")
